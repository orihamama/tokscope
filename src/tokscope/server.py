...
def overview(
    ...
    etag = (
        _etag() + "|" + ",".join(filter(None, [project, session_id, task_id, tool, since, until]))
    )
    if r := _maybe_304(req, etag):
        return r
...
def insights(
    ...
    etag = (
        _etag()
        + "|insights|"
        + ",".join(filter(None, [project, session_id, task_id, tool, since, until]))
    )
    if r := _maybe_304(req, etag):
        return r
...
def tools(
    ...
    etag = (
        _etag() + "|t|" + ",".join(filter(None, [project, session_id, task_id, tool, since, until]))
    )
    if r := _maybe_304(req, etag):
        return r
    c = _conn()
...
def tool_detail(name: str, req: Request):
    ...
    by_session = _rows(
        ...
        SELECT session_id, project, COUNT(*) calls,
               ROUND(SUM(attributed_cost_usd),4) cost
        FROM tool_calls WHERE tool_name=?
        GROUP BY session_id, project ORDER BY cost DESC LIMIT 20""",
        (name,),
    )
...
def tasks(
    ...
    if task_id:
        where_parts.append("root_tool_use_id = ?")
        params.append(task_id)
    if since:
        where_parts.append("started_at >= (strftime('%s', ?)*1000)")
        params.append(since)
    if until:
        where_parts.append("started_at <= (strftime('%s', ?)*1000)")
        params.append(until)
    ...
    by_type = _rows(
        ...
        FROM tasks{where} GROUP BY agent_type ORDER BY cost DESC""",
        tuple(params),
    )
    items = _rows(
        c,
...
def projects(
    ...
    etag = (
        _etag() + "|p|" + ",".join(filter(None, [project, session_id, task_id, tool, since, until]))
    )
    if r := _maybe_304(req, etag):
        return r
    c = _conn()
    ...
    items = _rows(
        ...
        FROM messages{msg_w} GROUP BY project ORDER BY cost DESC""",
        msg_p,
    )
    timeline = _rows(
        c,
    ...
    tool_mix = _rows(
        ...
        FROM tool_calls{tc_w} GROUP BY project, tool_name""",
        tc_p,
    )
...
def sessions_list(
    ...
    etag = (
        _etag() + "|s|" + ",".join(filter(None, [project, session_id, task_id, tool, since, until]))
    )
    if r := _maybe_304(req, etag):
        return r
    c = _conn()
    ...
    if until:
        ...
        params.append(until)
    if task_id:
        where_parts.append("session_id = (SELECT session_id FROM tasks WHERE root_tool_use_id=?)")
        params.append(task_id)
    if tool:
        where_parts.append(
...
def files(
    ...
    etag = (
        _etag() + "|f|" + ",".join(filter(None, [project, session_id, task_id, tool, since, until]))
    )
    if r := _maybe_304(req, etag):
        return r
    c = _conn()
...
def bash_stats(
    ...
    exit_codes = _rows(
        ...
        GROUP BY exit_code ORDER BY n DESC""",
        tc_p,
    )
    return _json(
        {
...
def search_stats(
    ...
    grep = _rows(
        ...
        GROUP BY search_pattern ORDER BY n DESC LIMIT 30""",
        g_p,
    )
    gl_w, gl_p = w("Glob")
    glob = _rows(
    ...
    web_fetch = _rows(
        ...
        GROUP BY web_url ORDER BY n DESC LIMIT 30""",
        wf_p,
    )
    ...
    web_search = _rows(
        ...
        GROUP BY web_query ORDER BY n DESC LIMIT 30""",
        ws_p,
    )
    return _json(
        {"grep": grep, "glob": glob, "web_fetch": web_fetch, "web_search": web_search}, etag
    )
...
def workflow(
    ...
    if project or session_id or task_id or since or until:
        ...
        bigrams = _rows(
            ...
            GROUP BY prev_tool, next_tool ORDER BY n DESC LIMIT ?""",
            tc_p + (limit,),
        )
    else:
        bigrams = _rows(
...
def health(
    ...
    sess_params: list = []
    if project:
        sess_w_parts.append("project = ?")
        sess_params.append(project)
    if session_id:
        sess_w_parts.append("session_id = ?")
    ...
    compactions = _rows(
        ...
        ORDER BY compaction_count DESC LIMIT 30""",
        tuple(sess_params),
    )
    error_tools = _rows(
        c,
...
def treemap(
    ...
    etag = (
        ...
        + "|tm|"
        + ",".join(
            filter(None, [project, session_id, task_id, tool, since, until, str(max_per_level)])
        )
    )
    if r := _maybe_304(req, etag):
        return r
    ...
    for p in proj_rows:
        ...
        if since:
            sub_where += " AND timestamp >= (strftime('%s', ?)*1000)"
            sub_params.append(since)
        if until:
            sub_where += " AND timestamp <= (strftime('%s', ?)*1000)"
            sub_params.append(until)
        if session_id:
            sub_where += " AND session_id = ?"
            sub_params.append(session_id)
        sess = _rows(
            c,
            f"""
            ...
            ORDER BY cost DESC LIMIT ?""",
            tuple(sub_params) + (max_per_level,),
        )
    ...
    if task_ids:
        ...
        agent_map_rows = c.execute(
            f"SELECT id, agent_id FROM tool_calls WHERE id IN ({','.join('?' * len(task_ids))})",
            task_ids,
        ).fetchall()
        ...
        if agent_to_task:
            ...
            tool_rows = _rows(
                ...
                WHERE m.agent_id IN ({",".join("?" * len(agent_ids))})
                  AND tc.attributed_cost_usd > 0
                GROUP BY m.agent_id, tc.tool_name""",
                tuple(agent_ids),
            )
            for r in tool_rows:
    ...
    def add_residual(
        parent_value: float, kids: list[dict], label: str = "(unattributed)"
    ) -> list[dict]:
    ...
    for p in proj_rows:
        ...
        for s in project_to_sessions.get(p["project"], []):
            ...
            for t in ch["tasks"]:
                ...
                tool_kids = [
                    child_node(tl["tool_name"], tl["cost"], "tool", calls=tl["calls"])
...
def treemap_residual(
    ...
    has_tool_msg_ids = (
        ...
        if msg_w
            ...
            r["uuid"]
            for r in c.execute("SELECT DISTINCT message_uuid uuid FROM tool_calls").fetchall()
        }
    )
    ...
    for m in msgs:
            ...
            cache_read_turns.append(
                {
                    ...
                    "hit_ratio": round(
                        (m["cache_read"] or 0)
                        / max(1, (m["cache_read"] or 0) + (m["input_tokens"] or 0)),
                        4,
                    ),
                }
            )
            ...
            thinking_turns.append(
                {
                    ...
                    "ratio": round(
                        (m["thinking_tokens"] or 0) / max(1, m["output_tokens"] or 0), 3
                    ),
                    "cost": round(tc, 4),
                }
            )
        if (
            m["role"] == "assistant"
            and m["uuid"] not in has_tool_msg_ids
            and (m["output_tokens"] or 0) > 0
        ):
        ...
        kids = [
            {
                "name": turn_label(
                    t["timestamp"],
                    f"{fmt_int(t['cache_read_tokens'])} read · hit {int(t['hit_ratio'] * 100)}%",
                ),
                "value": t["cost"],
                "kind": "residual_msg",
                "uuid": t["uuid"],
        ...
        children.append(
            {
                ...
                "summary": {
                    ...
                    "avg_hit_ratio": round(
                        sum(t["hit_ratio"] for t in cache_read_turns)
                        / max(1, len(cache_read_turns)),
                        4,
                    ),
    ...
    if cache_creation_cost > 0.001:
        ...
        kids = [
            {
                "name": turn_label(
                    t["timestamp"], f"{fmt_int(t['cache_creation_tokens'])} written"
                ),
                "value": t["cost"],
                "kind": "residual_msg",
                "uuid": t["uuid"],
        ...
        kids = [
            {
                "name": turn_label(
                    t["timestamp"],
                    f"{fmt_int(t['thinking_tokens'])} think · {int(t['ratio'] * 100)}% of out",
                ),
                "value": t["cost"],
                "kind": "residual_msg",
                "uuid": t["uuid"],
        ...
        children.append(
            {
                ...
                "summary": {
                    ...
                    "avg_ratio": round(
                        sum(t["ratio"] for t in thinking_turns) / max(1, len(thinking_turns)), 3
                    ),
                },
                "children": kids,
            }
    ...
    if input_total > 0.001:
        children.append(
            {
                "name": "input tokens (user msgs + system)",
                "value": round(input_total, 4),
                "kind": "residual_input",
            }
        )
...
def treemap_bash(
    ...
    if project:
        ...
        params.append(project)
    if since:
        where.append("timestamp >= (strftime('%s', ?)*1000)")
        params.append(since)
    if until:
        where.append("timestamp <= (strftime('%s', ?)*1000)")
        params.append(until)
...
def bash_programs(
    ...
    etag = (
        _etag()
        + "|bp|"
        + ",".join(filter(None, [project, session_id, task_id, since, until, str(limit)]))
    )
    if r := _maybe_304(req, etag):
        return r
    c = _conn()
...
def bash_program_detail(
    ...
    etag = (
        ...
        + name
        + "|"
        + ",".join(filter(None, [project, session_id, task_id, since, until]))
    )
    if r := _maybe_304(req, etag):
        return r
    c = _conn()
    ...
    samples = _rows(
        ...
        SELECT id, timestamp, is_error, exit_code,
               ...
               duration_ms
        FROM tool_calls{tc_w}{extra}
        ORDER BY attributed_cost_usd DESC LIMIT 30""",
        tc_p + (name,),
    )
    return _json(
        {"summary": dict(summary) if summary else {}, "subcommands": subs, "samples": samples}, etag
    )
...
def bd_sessions(req: Request, project: str, since: str | None = None, until: str | None = None):
    ...
    params: list = [project]
    if since:
        where.append("started_at >= (strftime('%s', ?)*1000)")
        params.append(since)
    if until:
        where.append("started_at <= (strftime('%s', ?)*1000)")
        params.append(until)
...
def bd_session_children(session_id: str, req: Request):
    ...
    direct_tools = _rows(
        ...
        GROUP BY tc.tool_name ORDER BY cost DESC""",
        (session_id,),
    )
