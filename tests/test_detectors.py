...
def test_paging_reads_finds_no_grep_session(seeded_db):
    rows = registry.detectors["paging_reads"].run(
        seeded_db, {}, {"min_pages": 5})
    # S1 has 5 paged Reads of /repo/big.cpp and zero Grep calls.
    assert any(r["file_path"] == "/repo/big.cpp" and r["pages"] == 5
               for r in rows)
...
def test_error_chains_excludes_user_rejections_by_default(seeded_db):
    ...
    rows = registry.detectors["error_chains"].run(
        seeded_db, {}, {"min_n": 1, "min_rate": 0.0})
    # Bash→Bash chain (T1 → T2 both errors).
    assert any(r["prev_tool"] == "Bash" and r["next_tool"] == "Bash"
               for r in rows)


def test_bash_retries_filters_rejections(seeded_db):
    rows = registry.detectors["bash_retries"].run(
        seeded_db, {}, {"window_s": 60})
    # Two `git status` calls within ~200ms, both with status_class='denied'.
    assert any(r["bash_command"] == "git status" and r["retries"] >= 2
               for r in rows)
...
def test_repeat_tasks_no_repeats_in_fixture(seeded_db):
    rows = registry.detectors["repeat_tasks"].run(
        seeded_db, {}, {"min_repeats": 2})
